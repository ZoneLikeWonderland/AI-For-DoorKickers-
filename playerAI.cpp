...
void Decide(){
    // bonus
    rep(i,2){
        if(dist(GetUnit(i).position,logic->map.bonus_places[i&1])>=bonus_radius)
        state[i]=RushToBonus;
        else
        {
    ...
    if(logic->crystal[logic->faction^1].belong==-1){
        fx(i,2,4){
            state[i]=RushToCrystal;
        }
    }else{
        if(dist(logic->crystal[logic->faction^1].position,logic->map.target_places[logic->faction])<dist(logic->crystal[logic->faction^1].position,logic->map.target_places[logic->faction^1])*0.6){
            fx(i,2,4){
                if(node_retranslate(logic->faction,logic->crystal[logic->faction^1].belong)==i){
...
// state evaluate
void Eval(int num){
    if(num==3&&dist(GetUnit(3).position,GetUnit(2).position)<splash_radius)
    state[num]=RandomAction;
    if(num==4&&(dist(GetUnit(4).position,GetUnit(2).position)<splash_radius||dist(GetUnit(4).position,GetUnit(3).position)<splash_radius))
    state[num]=RandomAction;
    ...
    case RushToBonus:
        ...
        break;
    case ProtectCrystal:
        if(dist(GetUnit(num).position,logic->crystal[logic->faction^1].position)>flash_distance){
            Forward(num,logic->crystal[logic->faction^1].position);
        }else{
